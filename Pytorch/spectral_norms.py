...
class SNHookConv(nn.Module):
    def __init__(self, input_ch, out_ch, ksize=None, stride=1, pad=0,
        nobias=False, initialW=None, initial_bias=None, dilate=1):
        super(SNHookConv, self).__init__()

        self.c =nn.Conv2d(input_ch, out_ch, kernel_size=ksize, stride=stride, padding=pad,
            bias=not nobias, dilation=dilate)
